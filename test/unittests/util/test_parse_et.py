...
class TestNormalize(unittest.TestCase):
    def test_articles(self):
        self.assertEqual(
            normalize("see on üks test", lang="et-ee", remove_articles=True),
            "see on 1 test")
    ...
    def test_extractdatetime_et(self):
...
#                    "2017-06-27 20:00:00", "lad os mødes")
    ...
    def test_extractdatetime_default_et(self):
        default = time(9, 0, 0)
        anchor = datetime(2017, 6, 27, 0, 0)
        res = extract_datetime("lad os mødes på fredag klokken 9 om morgenen",
                               anchor, lang='et-ee', default_time=default)
        self.assertEqual(default, res[0].time())

    def test_spaces(self):
        self.assertEqual(normalize("  dette   er   en   test", lang="et-ee"),
                         "dette er 1 test")
        self.assertEqual(normalize("  dette   er  en   test  ",
                                   lang="et-ee"), "dette er 1 test")

    def test_numbers(self):
        self.assertEqual(
            normalize("see on üks kaks kolm test", lang="et-ee"),
            "see on 1 2 3 test")
        self.assertEqual(
            normalize("see on viisteist kuusteist test", lang="et-ee"),
            "see on 15 16 test")
