    ...
    while count < len(aWords):
        ...
        if is_numeric(word):
            if word.isdigit():            # doesn't work with decimals
                val = float(word)
        elif isFractional_et(word):
            val = isFractional_et(word)
        elif isOrdinal_et(word):
            val = isOrdinal_et(word)
        else:
            if word in et_numbers:
                val = et_numbers[word]
                if count < (len(aWords) - 1):
                    wordNext = aWords[count + 1]
                else:
                    wordNext = ""
                valNext = isFractional_et(wordNext)
...
def extract_datetime_et(string, currentDate, default_time):
    def clean_string(s):
        ...
        for idx, word in enumerate(wordList):
            if isOrdinal_et(word) is not False:
                word = str(isOrdinal_et(word))
                wordList[idx] = word
...
def isFractional_et(input_str):
    ...
    """
    if input_str.lower().startswith("pool"):
        return 0.5

    if input_str.lower() == "kolmandik":
        return 1.0 / 3
    elif input_str.endswith('del'):
        input_str = input_str[:len(input_str) - 3]  # e.g. "fünftel"
        if input_str.lower() in et_numbers:
            return 1.0 / (et_numbers[input_str.lower()])

    return False


def isOrdinal_et(input_str):
    """
    ...
    only works for ordinals corresponding to the numbers in et_numbers
...
def extract_numbers_et(text, short_scale=True, ordinals=False):
    ...
    """
    return extract_numbers_generic(text, pronounce_number_et, extractnumber_et,
